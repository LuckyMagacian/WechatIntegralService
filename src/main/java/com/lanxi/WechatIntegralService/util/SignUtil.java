...
public class SignUtil {
	...
	public static String md5LowerCase(String src, String charset) {
		...
		try {
			if (src == null || src.length() < 1)
				throw new AppException("md5摘要�?,明文信息为null或为�?");
	...
	public static String base64En(String src, String charset) {
		try {
			if (src == null || src.length() < 1)
				throw new AppException("bas64加密�?,明文信息为null或为�?");
	...
	public static String base64De(String src, String charset) {
		try {
			if (src == null || src.length() < 1)
				throw new AppException("bas64解密�?,明文信息为null或为�?");
	...
	public static String byteToHex(byte[] bytes) {
		try {
			if (bytes == null || bytes.length < 1)
				throw new AppException("byte转Hex�?,byte数组为null或为�?");
			return new String(Hex.encodeHex(bytes));
		} catch (Exception e) {
			throw new AppException("byte转hex时异�?", e);
		}
	}
	...
	// ------------------------------------signStart------------------------------------------
	/**
	 * map转String key=value&key2=value2 不排�? 无签�?
	 * 
	 * @param map
	 * @return
	 */
	...
	/**
	 * map转String key=value&key2=value2 不排�? 有签�?
	 * 
	 * @param map
	 * @return
	 */
	...
	/**
	 * map转String key=value&key2=value2 key按字典排�? 无签�?
	 * 
	 * @param map
	 * @return
	 */
	...
	/**
	 * map转String key=value&key2=value2 key按字典排�? 有签�?
	 * 
	 * @param map
	 * @return
	 */
	...
	public static Map<String, Object> getKeyPair() {
		try {
			...
			throw new AppException("获取密钥对异�?", e);
		}
	}
	...
	public static byte[] rsaEn(PublicKey key, byte[] data) {
		try {
			if (data.length > key.getEncoded().length - 11)
				throw new AppException("加密内容过长,不能大于245个字�?");
	...
	// /**
	// * rsa公钥加密
	// * 该方法容易出现异�?-原因是输入的字符串内容转字节时可能会变成两个字节
	// * @param pubKey
	// * @param content
	// * @param charset
	// * @return
	...
	// } catch (Exception e) {
	// throw new AppException("字符串转字节时编码异�?", e);
	// }
	// }
	...
	// /**
	// * rsa 私钥解密
	// * 该方法容易出现异�?,原因�? 字符串转字节�?,对于部分字符可能会变成两个字�?
	// * @param priKey
	// * @param content
	// * @param charset
	// * @return
	...
	public static String rsaMd5Sign(String priKey, byte[] data) {
		try {
			...
			throw new AppException("字符串转字节是编码异�?", e);
		}
	}
//	/**
//	 * rsa md5 获取签名
